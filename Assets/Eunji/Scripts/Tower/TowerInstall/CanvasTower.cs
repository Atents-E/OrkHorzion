...
public class CanvasTower : MonoBehaviour
{
    ...
    public void OnRemove(InputAction.CallbackContext _)
    {
        ...
        ////if (Physics.Raycast(ray, out RaycastHit hit, 100.0f, LayerMask.GetMask("Tower"))) // 레이와 타워의 충돌 여부 확인
        ////{

        //    raycastHit = Physics.RaycastAll(ray, 1000.0f, LayerMask.GetMask("Tower"));

        //    for(int i =0; i > raycastHit.Length; i++)
        //    {
        //        RaycastHit towerHit = raycastHit[i];
        //        Debug.Log($"{towerHit}");

        //        if(towerHit.collider.isTrigger != true)
        //        {
        //            // all로 하고 트리거가 아닌 애를 기준으로 ..
        //            Debug.Log($"{towerHit.collider.gameObject.name}");
        //            // 충돌한 지점에 오브젝트가 있다
        ...
        //            tower = towerHit.collider.gameObject;
