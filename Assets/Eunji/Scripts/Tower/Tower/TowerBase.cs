...
public class TowerBase : MonoBehaviour
{
    ...
    //protected Transform direction;                    // 타워 방향
    protected Transform directionTransform;           // 투사체 트랜스폼

    protected Transform target = null;                // 타겟은 null
    ...
    protected virtual void LookTarget()   // 타겟을 봐라
    {
        if (target != null)     // 타겟이 있다면,
        {
            // 각도를 사용하는 경우(등속도로 회전)
            Vector3 shotToMonsterDir = target.position
                - transform.position;  // 방향(타워)에서 적의 위치로 가는 방향 벡터 계산
    ...
    {
        Vector3 dir = target.position - transform.position;         // 타겟 위치 - 방향 위치
    ...
    protected virtual IEnumerator PeriodFire()    // 발사 기간
    {
    ...
    protected virtual void OnDrawGizmos/*Selected*/() // 기즈모
    {
#if UNITY_EDITOR
