...
// ItemData용 커스텀 에디터를 작성한한다는 표시, 두번쨰 파라메터를 true로 해서 자식 클래스도 같이 적용받게 만들기
[CustomEditor(typeof(ItemData_Base), true)]
public class ItemDataEditor : Editor
{
    // 선택된 ItemData를 저장할 변수
    ItemData_Base itemData;

    private void OnEnable()
    {
        // target은 에디터에서 선택한 오브젝트
        // target을 ItemData로 캐스팅 시도. 성공하면 null이 아닌 참조값. 실패하면 null
        itemData = target as ItemData_Base;
    }
